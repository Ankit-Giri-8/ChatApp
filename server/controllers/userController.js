...
export const signup = async (req,res)=>{
    ...
    try {
        if (!fullName || !email || !password) {
            return res.json({ success: false, message: "Missing Details details" });
        }
        if (!bio) {
            return res.json({ success: true, message: "Details Done. Add bio..." });
        }
    ...
    try {
        ...
        if (!profilePic) {
          updatedUser = await User.findByIdAndUpdate(
            userId,
            { profilePic: DEFAULT_AVATAR, bio, fullName },
            { new: true }
          );
        } 
        else {
